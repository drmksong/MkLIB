...
// Completly empty polygon, no memory allocated for points, no points, no lines...
...
// Memory for points are allocated but points are not defined, i.e. remain at origin(0,0,0)
...
bool MkPolygon::FormPolyLines() // TODO: return always true, which is to be revised, either make it void or useful
{
  // TODO: Form Poly Lines for the entire Polygon Class and assure that formed polygon is used and updated when necessary
  assert(FLines.GetSize() == FSize);

  for (int i; i < FSize; i++)
  {
    if (i < FSize - 1)
    {
      FLines[i] = (MkLine(FPoint[i], FPoint[i + 1])); // call by value, = is for call by reference, 23.01.26
    }
    else if (i == FSize - 1)
    {
      FLines[i] = (MkLine(FPoint[i], FPoint[0])); // call by value, = is for call by reference, 23.01.26
    }
  }
  return true;
...
void MkPolygon::getCrossWithZ(double z, MkPoints &pnts)
{
  int i, cnt = 0;
  for (i = 0; i < FSize; i++)
  {
  ...
  for (i = 0; i < FSize; i++)
  {
    ...
    if (min(a.Z, b.Z) < z && z < max(a.Z, b.Z))
    {
      pnts[cnt].X = a.X + (b.X - a.X) / (b.Z - a.Z) * (z - a.Z);
      pnts[cnt].Y = a.Y + (b.Y - a.Y) / (b.Z - a.Z) * (z - a.Z);
      pnts[cnt].Z = z;
      cnt++;
    }
  }
...
double MkPolygon::getXWithY(double y)
{
  int i;
  double value;
  MkPoints pnts;
  getCrossWithY(y, pnts);
  if (pnts.GetSize())
    value = pnts[0].X;
  else
    return 0;
  for (i = 1; i < pnts.GetSize(); i++)
  {
    ...
    {
    case coMax:
      value = value > pnts[i].X ? value : pnts[i].X;
      break;
    case coAbsMax:
      value = fabs(value) > fabs(pnts[i].X) ? value : pnts[i].X;
      break;
    case coMin:
      value = value < pnts[i].X ? value : pnts[i].X;
      break;
...
double MkPolygon::getXWithZ(double z)
{
  int i;
  double value;
  MkPoints pnts;
  getCrossWithZ(z, pnts);
  if (pnts.GetSize())
    value = pnts[0].X;
  else
    return 0;
...
double MkPolygon::getYWithX(double x)
{
  int i;
  double value;
  MkPoints pnts;
  getCrossWithX(x, pnts);
  if (pnts.GetSize())
    value = pnts[0].Y;
  else
    return 0;
  for (i = 1; i < pnts.GetSize(); i++)
  {
    ...
    {
    case coMax:
      value = value > pnts[i].Y ? value : pnts[i].Y;
      break;
    case coAbsMax:
      value = fabs(value) > fabs(pnts[i].Y) ? value : pnts[i].Y;
      break;
    case coMin:
      value = value < pnts[i].Y ? value : pnts[i].Y;
      break;
...
double MkPolygon::getYWithZ(double z)
{
  int i;
  double value;
  MkPoints pnts;
...
double MkPolygon::getZWithX(double x)
{
  int i;
  double value;
  MkPoints pnts;
  ...
  for (i = 1; i < pnts.GetSize(); i++)
  {
    ...
    {
    case coMax:
      value = value > pnts[i].Z ? value : pnts[i].Z;
      break;
    case coAbsMax:
      value = fabs(value) > fabs(pnts[i].Z) ? value : pnts[i].Z;
      break;
    case coMin:
      value = value < pnts[i].Z ? value : pnts[i].Z;
      break;
...
double MkPolygon::getZWithY(double y)
{
  int i;
  double value;
  MkPoints pnts;
...
void MkPolygon::Sort(MkPolygonSort ps)
{
  int i, j;
  bool flag;
  for (i = 0; i < FSize - 1; i++)
  {
    for (j = i + 1; j < FSize; j++)
    {
      ...
      {
      case psXA:
        flag = FPoint[i].X < FPoint[j].X;
        break;
      case psXD:
        flag = FPoint[i].X > FPoint[j].X;
        break;
      case psYA:
        flag = FPoint[i].Y < FPoint[j].Y;
        break;
      case psYD:
        flag = FPoint[i].Y > FPoint[j].Y;
        break;
      case psZA:
        flag = FPoint[i].Z < FPoint[j].Z;
        break;
      case psZD:
        flag = FPoint[i].Z > FPoint[j].Z;
        break;
...
MkLine &MkPolygon::operator()(int i)
{
  if (i >= 0 && i < GetSize() - 1)
    ...
    return MkLine((*this)[i], (*this)[i + 1], Color);
#else
    // return MkLine((*this)[i], (*this)[i + 1]);
    return FLines[i];
#endif
  else if (Closeness && i == GetSize() - 1)
#ifdef __BCPLUSPLUS__
    return MkLine((*this)[i], (*this)[0], Color);
#else
    // return MkLine((*this)[i], (*this)[0]);
    return FLines[0];
#endif
  else
    return NullLine;
...
MkPolygon &MkPolygon::operator=(MkPolygon &polygon)
{
  ...
  doDrawPoint = polygon.doDrawPoint;
  doFill = polygon.doFill;
...
{                                                                                   // extract subpolygon between the depth y1 and y2
  ...
  double _ymax = -1.0e20, _ymin = 1.0e20;
  bool flag;
  ...
  pmax = pmax + fabs(pmax * 0.1 + 0.1);
  pmin = pmin - fabs(pmin * 0.1 + 0.1);
  ...
  {
    flag = false;
    for (i = 0; i < outpoly.GetSize() - 1; i++)
    {
      for (j = i + 1; j < outpoly.GetSize(); j++)
      {
        if (outpoly[i] == outpoly[j])
        {
          outpoly.Delete(j);
          flag = true;
          break;
